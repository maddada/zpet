...
struct Cli {
    ...
    /// [deprecated: use `gte config show`]
    #[arg(long, hide = true)]
    dump_config: bool,

    /// [deprecated: use `gte config paths`]
    #[arg(long, hide = true)]
    show_paths: bool,
...
fn list_sessions_command() -> AnyhowResult<()> {
    ...
    if sessions.is_empty() {
        ...
        if sessions.len() == 1 {
            ...
            if display != id {
                println!("attach with: gte -a  (from that directory)");
                println!("         or: gte -a {}", id);
            } else {
                println!("attach with: gte -a {}", id);
            }
        } else {
            println!("attach with: gte -a [name]");
        }
    }
...
fn run_open_files_command(
    ...
    if server_was_started {
        ...
        if std::io::IsTerminal::is_terminal(&std::io::stdin()) {
            ...
            eprintln!(
                "started new session and opened {} file(s). attach with: gte -a{}",
                file_requests.len(),
...
fn run_attach(session_name: Option<&str>, files: &[String]) -> AnyhowResult<()> {
    ...
    match result {
        ...
        Ok(client::ClientExitReason::Detached) => {
            ...
            eprintln!("reattach with: gte -a  or  gte session attach");
        }
...
fn print_deprecation_warnings(cli: &Cli) {
    ...
    if cli.dump_config {
        eprintln!("warning: --dump-config is deprecated, use `gte --cmd config show` instead");
    }
    if cli.show_paths {
        eprintln!("warning: --show-paths is deprecated, use `gte --cmd config paths` instead");
    }
    if cli.init.is_some() {
        eprintln!("warning: --init is deprecated, use `gte --cmd init` instead");
    }
}
...
fn build_localized_after_help() -> String {
    ...
    out.push_str(&format!(
        "  gte 'file.txt:10-20@\"Check this code\"'     {}\n",
    ...
    out.push_str(&format!(
        "  gte --cmd session new proj                 {}\n",
        t("cli.example.new_session")
    ));
    out.push_str(&format!(
        "  gte --cmd session open-file . main.rs     {}\n",
        t("cli.example.open_in_dir")
    ));
    out.push_str(&format!(
        "  gte --cmd session open-file proj a.rs     {}\n",
    ...
    out.push_str("    gte --cmd session open-file . 'src/main.rs:10-25@\"msg\"' --wait\n\n");
    out.push_str(&format!("  {}\n", t("cli.guided.markdown")));
    out.push_str(
        "    gte --cmd session open-file . \\\n      $'src/main.rs:10-25@\"**Title**\\nBody text here\"' --wait\n\n",
    );
    out.push_str(&format!("  {}\n", t("cli.guided.walkthrough")));
    out.push_str("    gte --cmd session open-file . 'a.rs:1-10@\"Step 1\"' --wait\n");
    out.push_str("    gte --cmd session open-file . 'b.rs:5-20@\"Step 2\"' --wait\n");
    out.push_str("    gte --cmd session open-file . 'c.rs:30@\"Step 3\"'   --wait\n\n");
    out.push_str(&format!("  {}\n", t("cli.guided.git_editor")));
    out.push_str("    git config core.editor 'gte --cmd session open-file . --wait'\n\n");
...
fn real_main() -> AnyhowResult<()> {
    ...
    if let Some(update_result) = last_update_result {
        if update_result.update_available {
            eprintln!();
            eprintln!(
                "A new version of gte is available: {} -> {}",
